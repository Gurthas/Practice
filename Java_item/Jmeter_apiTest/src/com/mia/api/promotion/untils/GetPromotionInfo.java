...
public class GetPromotionInfo extends AbstractJavaSamplerClient {
	...
	private static int SERVER_PORT_DEFAULT = 9090;
	// 定义测试参数
	private String Param_List;
	private static String Param_Name = "Param_List";
	private static String Param_List_DEFAULT = "1000024,1000025";
	...
	public SampleResult runTest(JavaSamplerContext context) {
		...
		results.setSampleLabel("getPromotionInfo");
		setupValues(context);
		Param_List = context.getParameter(Param_Name);
		QueryParam params = _QueryParam.params(Param_List);
		RPromotionList result = null;
		
		
		// 定义一个事务，表示这是事务的起始点
		results.sampleStart();
		try {
			result = client.getPromotionInfo(params);
		} catch (TException e) {
			e.printStackTrace();
		}
		...
		if (result == null) {
			...
			results.setResponseData("Response Body" + result.getCode());
			results.setResponseMessage("false");
			results.setSuccessful(false);
		}
		return results;
