...
public class JsonParse {
	private static Logger log = LogManager.getLogger("JsonParse");
	...
	/*
	 * 文件解析，从文件中读取json数据，只适合标准json，即不含有JSONArray、jsonobject Json 解析方法 : 关键字解析
	 * result Json原始字符串 path Json标签位置信息：content/default_address/id
	 */
	public static String File(String filename,String key) {
		...
		try {
			for (int i = 0; i < maptxt.length; i++) {
				if (i == (maptxt.length - 1)) {
					code = Json.get(maptxt[i]);
					break;
				}
				code = Json.getJSONObject(maptxt[i]);
				Json = JSON.parseObject(code.toString());
			}
		} catch (Exception e) {
			code = null;
			log.info("Json解析失败：" + Json + "\n失败key：" + key);
		}
