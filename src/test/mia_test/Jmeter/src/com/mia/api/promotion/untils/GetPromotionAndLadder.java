...
import com.mia.api.promotion.RPromotionAndLadder;
...
public class GetPromotionAndLadder extends AbstractJavaSamplerClient {
	...
	public SampleResult runTest(JavaSamplerContext context) {
		...
		results.setSampleLabel("getPromotionAndLadder");
		setupValues(context);
		Param_List = context.getParameter(Param_Name);
		QueryParam params = _QueryParam.params(Param_List);
		RPromotionAndLadder result = null;
		// 定义一个事务，表示这是事务的起始点
		results.sampleStart();
		try {
			result = client.getPromotionAndLadder(params);
		} catch (TException e) {
			e.printStackTrace();
		}
		// 定义事务，标志事务结束
